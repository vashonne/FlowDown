...
private enum QuickReplyIntentHandler {
    struct Options {
    ...
    private static func preparePrompt() async -> String {
        await MainActor.run {
            let manager = ModelManager.shared
    ...
    private static func prepareImageContentPart(from file: IntentFile) throws -> ChatRequestBody.Message.ContentPart {
        var data = file.data
...
struct GenerateChatResponseIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult & ReturnsValue<String> & ProvidesDialog {
        let response = try await QuickReplyIntentHandler.execute(
            model: model,
            message: message,
            image: nil,
